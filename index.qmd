...
::: {style="height: 450px; overflow-y: scroll; border: 1px solid #4B9CD3; border-radius: 6px;"}
<a href="https://russblessing.github.io/positron-quarto/manuscript/" target="_blank"> <img src="assets/screenshots/rendered_html.png" style="width: 100%;"/> </a>
:::
...
## Longleaf Configuration: Load Modules

- Go to: https://ondemand.rc.unc.edu/
- Select: Files \> Home
- Click: \>\_Open in Terminal
- copy/paste this into terminal and hit enter

``` bash
module purge
module add r/4.5.0
module add gdal/3.11.0
```

## Longleaf Configuration: Install `units` from Source

The current CRAN release doesn't build cleanly — use the archived version instead:

```bash
mkdir -p ~/R/x86_64-pc-linux-gnu-library/4.5
wget https://cran.r-project.org/src/contrib/Archive/units/units_0.8-7.tar.gz
R CMD INSTALL units_0.8-7.tar.gz
```

## Longleaf Configuration: Install supporting packages

Start R and install normally from CRAN:

```r
R
#then, seperate line as new entry
> install.packages(c('classInt', 's2'))
```

::: {.fragment}
Choose the **USA (IA) [https]** mirror if prompted.
:::

::: {.fragment}
When you quit (`q()`), select **n** — don't save the workspace image.
:::

## Longleaf Configuration: Install `sf` from Source 

```bash
wget https://cran.r-project.org/src/contrib/sf_1.0-24.tar.gz
```

Then, all on one line:

```bash
R CMD INSTALL sf_1.0-24.tar.gz --configure-args='--with-gdal-config=/nas/longleaf/rhel9/apps/gdal/3.11.0/bin/gdal-config --with-geos-config=/nas/longleaf/rhel9/apps/geos/3.12.0/bin/geos-config --with-proj-lib=/nas/longleaf/rhel9/apps/proj/9.2.1/lib64 --with-proj-include=/nas/longleaf/rhel9/apps/proj/9.2.1/include --with-sqlite3-lib=/nas/longleaf/rhel9/apps/sqlite/3.39.0/lib'
```


## Install Quarto on Longleaf

``` bash
cd ~
wget https://github.com/quarto-dev/quarto-cli/releases/download/v1.8.27/quarto-1.8.27-linux-amd64.tar.gz
tar -xzvf quarto-1.8.27-linux-amd64.tar.gz
ln -s $PWD/quarto-1.8.27/bin/quarto ~/bin/quarto
```


## Setting up Positron

Edit your ssh file on your local machine from terminal:
...
In Positron, go to positron \> settings \> and type: `kernel supervisor: run` - Select the checkbox for "Run Kernels in a login shell" ![](assets/screenshots/Supervisor.png)
...
![](assets/screenshots/ssh.png) Type: `<<cluster id>>.ll.unc.edu` and hit enter

## Create a Quarto Project

In Positron, go to New \> New Folder from Git \> and paste the URL of your GitHub repository Next, create a Quarto Manuscript Project within the directory
...
- Use the `references.bib` file to manage your citations
- Use the notebooks folder to create and organize your data analysis notebooks as qmd files

## Manucript Formatting & Publishing

Many journals now have their own Quarto template for manuscript formatting, found [here](https://github.com/quarto-journals/). Follow their instructions for adding the template as an extension. Before publishing go to your GitHub repository.
